...
public class ScreenshotCompanion : MonoBehaviour
{
    ...
    {
        // This will trigger when the assigned action (hotkey) is performed.
        TakeScreenshot();
    }
    ...
    {
        // Unsubscribe from the action when the object is disabled or destroyed
        debugInputActions.Debug.DebugAction.performed -= OnDebugAction;
        debugInputActions.Disable();
    }
    */

    private void TakeScreenshot()
    {
        // Loop through the list and check hotkey bindings
        for (int i = 0; i < list.Count; i++)
        {
            ...
            {
                Debug.LogError("Screenshot by Hotkey could not be created! Camera not available.");
            }
        }
    }
